...
def janusgraph_service(action='start'):
  ...
  if action == 'start':
    ...
    janusgraph_x_sparkjars_exist_command = format("ls {janusgraph_ext_spark_plugin_dir}/*.jar>/dev/null 2>&1")
    Execute(janusgraph_x_spark_targz_command,
            not_if = janusgraph_x_sparkjars_exist_command,
            logoutput=True,user=params.janusgraph_user)

    #delete spark targz
    janusgraph_remove_spark_targz_command = format("rm -f {janusgraph_ext_spark_plugin_dir}/{janusgraph_spark2_archive_file}")
    janusgraph_sparkjars_still_exist_command= format("ls {janusgraph_ext_spark_plugin_dir}/{janusgraph_spark2_archive_file}>/dev/null 2>&1")
    janusgraph_sparkjars_deleted_command = format("[[ $? == 1 ]]")
    Execute(janusgraph_remove_spark_targz_command,
            not_if = janusgraph_sparkjars_deleted_command,
            logoutput=True,user=params.janusgraph_user)
    ...
    #create file for gremlin-server-metrics.csv
    metrics_file = params.janusgraph_server_csv_metrics
    janusgraph_create_metrics_dir_command = format("mkdir -p {janusgraph_server_csv_metrics_dir}")
    janusgraph_metrics_dir_exist_command = format("ls {janusgraph_server_csv_metrics_dir}>/dev/null 2>&1")
    Execute(janusgraph_create_metrics_dir_command,
            not_if = janusgraph_metrics_dir_exist_command,
            logoutput=True,user=params.janusgraph_user)
